...
export const BuildProject = ( magic_config ) => {
	const build_config = magic_config.build;
	...
	runTask( "project init build", () => {
		...
		if ( build_config[ "exclude" ].length > 0 ) rulesFile.push( ...build_config[ "exclude" ] );
		if ( build_config[ "excludeDir" ].length > 0 ) rulesFile.push( ...build_config[ "excludeDir" ] );
		...
		try {
			...
			files.forEach( ( filePath ) => {
				...
				if ( ext === ".m" ) {
					...
					const filePath = path.normalize( `${ build_dir }/m/${ name }` );
					File.createFileWithDirectories( filePath, data );
					file_to_build[ ext ].push( filePath );
				} else {
